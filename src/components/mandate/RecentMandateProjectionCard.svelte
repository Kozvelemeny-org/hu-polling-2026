<script lang="ts">
    import type { MandateProjection, Party } from "$lib/types";
    import { pollsterData, partyData } from "$stores/dataStore";
    
    export let poll: MandateProjection;
    export let selectedParties: Party[] = [];

    let mandateProjectionData = {
        date: new Date(),
        dateDisplay: "",
    ...
    $: {
        ...
        const partyResults = selectedParties
            .map(party => ({
                party,
                value: poll[party] ?? 0,
                displayValue: Number(poll[party] ?? 0),
...
<a href={poll.url} target="_blank"><article>
    <header>
        ...
        <div class="date">
            {mandateProjectionData.dateDisplay} 
        </div>
    </header>
    {#if mandateProjectionData.partyResults.length > 0}
    <section>
        {#each mandateProjectionData.partyResults as result}
            <div
                class="valueBackground"
                style="width: {result.displayValue * 100 / 199 * (result.displayValue > 10 ? 1.5 : 1.5)}%;"
            >
                <div 
